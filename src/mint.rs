...
impl Contract {
    ...
    // BURN NFT  --- > SEND NFT TO ZERO ADDRESS
    #[payable]
    pub fn nft_burn(&mut self, token_id: TokenId) -> BurnNFTJson {
        assert_one_yocto();
...
// For Airdroping NFT
 ...
 pub fn airdop(  
     ...
     assert!(
        num_to_mint == receiver_id.len() as u64,
        " NUM_TO_MINT  SHOULD BE EQUAL TO ADDRESSES"
    );
     ...
     (0..num_to_mint).for_each(|i| {
         ...
         let token = Token {
             //set the owner ID equal to the receiver ID passed into the function
             owner_id: receiver_id[type_cast].clone(),
             //we set the approved account IDs to the default value (an empty map)
             approved_account_ids: Default::default(),
             //the next approval ID is set to 0
             next_approval_id: 0,
         ...
         self.token_metadata_by_id
             .insert(&token_id.to_string(), &metadata[type_cast]);
